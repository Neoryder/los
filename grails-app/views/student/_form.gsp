...
<div class="fieldcontain ${hasErrors(bean: studentInstance, field: 'teacher', 'error')} required">
	...
	<g:select id="teacher" name="teacher.id" noSelection="${['null':'Select One...']}" from="${los.Teacher.list()}" optionKey="id" optionValue="assignedNumber" required="" value="${studentInstance?.teacher?.id}" class="many-to-one"/>
</div>
